...
if (!is_null($page_clairvoyance)) {
	...
	$scraper_page .= '<a class="btn btn-light btn-block rounded-pill border border-secondary" href="' . get_the_permalink($page_clairvoyance->ID) . '">' . get_the_title($page_clairvoyance->ID) . '</a>';
	$scraper_page .= '</div>';
}
// Get seervices
$args = array(
				...
				<div class="row text-center">
					...
					<?php while ($services->have_posts()) :
						...
						<div class="col-12 col-sm-6 col-md-4 col-lg-3 col-xl-2 mb-2">
							<a class="btn btn-light btn-block rounded-pill border border-secondary" href="<?php echo get_home_url() . '/' . $post->post_name; ?>/"><?php the_title(); ?></a>
						</div>
					<?php endwhile; ?>
				</div>
			</div>
